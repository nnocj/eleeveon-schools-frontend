...
function schoolIdOf(row: AnyRow) {
  return cleanId(row?.schoolId ?? row?.payload?.schoolId);
}
...
function isTargetedToBranch(row: AnyRow, branchId?: string | null) {
  const targetBranchId = cleanId(branchId);
  if (!targetBranchId) return false;

  const ids = Array.isArray(row?.targetBranchIds) ? row.targetBranchIds : [];
  return ids.some((value: unknown) => cleanId(value) === targetBranchId);
}
...
export default function Announcements() {
  ...
  async function load() {
    ...
    try {
        ...
        (ann as AnyRow[])
          .filter(
            (row) =>
              ...
              isTargetedToBranch(row, branchId),
          )
      ...
      setTeachers(
        (tea as AnyRow[]).filter((row) =>
          isBranchRow(row, accountId, schoolId, branchId),
        ),
      );
      setParents(
        (par as AnyRow[]).filter((row) =>
          isBranchRow(row, accountId, schoolId, branchId),
        ),
      );
      setStudents(
        (stu as AnyRow[]).filter((row) =>
          isBranchRow(row, accountId, schoolId, branchId),
        ),
      );
      setMemberships(
        (memRaw as AnyRow[]).filter((row) =>
          isBranchRow(row, accountId, schoolId, branchId),
        ),
      );
  ...
  const candidates = useMemo(() => {
    ...
    if (form.audience === "accountants" || form.audience === "whole_branch") {
      memberships
        .filter(
          (membership) =>
            String(membership.role || "").toLowerCase() === "accountant",
        )
        .forEach((membership) => {
          const user =
            users.find(
              (item) =>
                String(item.id) ===
                String(
                  membership.userId || membership.accountUserId,
                ),
            ) ||
            users.find(
              (item) =>
    ...
    return rows.filter((row) => {
      const key = String(
        row.userId || row.email || row.phone || `${row.role}-${row.profileId}`,
      );
  ...
  const filtered = useMemo(() => {
    ...
    return announcements
      .filter(
        (announcement) =>
          status === "all" || String(announcement.status || "sent") === status,
      )
      .filter(
        (announcement) =>
          audienceFilter === "all" ||
          normalizeAudience(
            announcement.audience || announcement.targetRole,
          ) === audienceFilter,
      )
      .filter(
        (announcement) =>
          priorityFilter === "all" ||
          String(announcement.priority || "normal") === priorityFilter,
      )
      .filter((announcement) => {
        if (!q) return true;
        return [
      ...
      .sort(
        (a, b) =>
          n(b.sentAt || b.publishAt || b.createdAt) -
          n(a.sentAt || a.publishAt || a.createdAt),
      );
  ...
  const summary = useMemo(
    () => ({
      ...
      sent: announcements.filter(
        (row) => String(row.status || "sent") === "sent",
      ).length,
      draft: announcements.filter((row) => String(row.status || "") === "draft")
        .length,
      scheduled: announcements.filter(
        (row) => String(row.status || "") === "scheduled",
      ).length,
      urgent: announcements.filter((row) =>
        ["urgent", "high"].includes(String(row.priority || "")),
      ).length,
    ...
    () =>
      AUDIENCES.map((audience) => ({
        ...
        value: announcements.filter(
          (item) =>
            normalizeAudience(item.audience || item.targetRole) ===
            audience.value,
        ).length,
      })),
    [announcements],
  ...
  const activeFilterCount = useMemo(() => {
    return [status, audienceFilter, priorityFilter].filter(
      (item) => item !== "all",
    ).length;
  ...
  function openEdit(announcement: AnyRow) {
    ...
    setForm({
      ...
      channels: normalizeChannels(
        announcement.channels || announcement.channel,
      ),
    });
    setDrawer(true);
  }
  ...
  async function replaceRecipients(announcementId: string) {
    const existing = recipients.filter(
      (recipient) =>
        String(recipient.announcementId) === String(announcementId),
    );

    for (const recipient of existing) {
      const id = cleanId(recipient.id);
      if (id) await softDeleteLocal("announcementRecipients" as any, id);
    }

    for (const recipient of candidates) {
      await createLocal(
        "announcementRecipients" as any,
        {
  ...
  async function save() {
    ...
    if (!candidates.length)
      return showToast("error", "No recipients found for this audience.");
    ...
    try {
      ...
      if (drawerMode === "edit" && announcementId) {
        ...
        const created = (await createLocal("announcements" as any, payload)) as
          | AnyRow
          | undefined;
      ...
      showToast(
        "success",
        drawerMode === "edit" ? "Announcement updated." : "Announcement saved.",
      );
      await load();
    } catch (error: any) {
  ...
  async function deleteAnnouncement(announcement: AnyRow) {
    const id = cleanId(announcement.id);
    if (!id) return;

    const ok = window.confirm(
      `Delete "${announcement.title || "announcement"}"?`,
    );
    if (!ok) return;
    ...
    try {
      ...
      const linked = recipients.filter(
        (recipient) => String(recipient.announcementId) === String(id),
      );
      for (const recipient of linked) {
        const recipientId = cleanId(recipient.id);
        if (recipientId)
          await softDeleteLocal("announcementRecipients" as any, recipientId);
      }
  ...
  if (loading || accountLoading || settingsLoading || contextLoading) {
    return (
      <State
        ...
        text="Loading branch notices and recipients."
      />
    );
  }
  ...
  if (!authenticated || !accountId) {
    return (
      <State
        ...
        text="You must sign in before managing announcements."
      />
    );
  }
    ...
    return (
      ...
      >
        ...
        <section className="ba-state">
          ...
          <p>
            Announcements belong to the selected branch-admin workspace. Use
  ...
  return (
    ...
    >
      ...
      <section
        ...
        aria-label="Announcements search and actions"
      >
      ...
      {activeFilterCount > 0 && (
        <section className="ba-filter-chips" aria-label="Active filters">
          ...
          {audienceFilter !== "all" && (
            <button type="button" onClick={() => setAudienceFilter("all")}>
              Audience: {audienceLabel(audienceFilter)} ×
            </button>
          )}
          {priorityFilter !== "all" && (
      ...
      {view === "analytics" && (
        <section className="ba-analysis-grid">
          <AnalysisCard
            title="Announcements by Audience"
            rows={audienceCounts}
          ...
          <article className="ba-analysis">
            ...
            <p>Recipient records generated for branch announcements.</p>
          </article>
          <article className="ba-analysis">
            <span>Current Filter</span>
            <strong>{summary.shown}</strong>
            <p>
              Announcement record(s) currently match your search and filters.
            </p>
          </article>
        </section>
      )}
        ...
        <section className="ba-list">
          ...
          {!filtered.length && (
            <Empty
              ...
              text="Create branch announcements for teachers, parents, students, accountants, or the whole branch."
            />
          )}
        </section>
      )}
...
function AnnouncementListItem({
  ...
  deleteAnnouncement: (row: AnyRow) => void;
}) {
  return (
    <article className="announcement-row">
      ...
      <span className="announcement-main">
        ...
        <small>
          ...
          · {audienceLabel(announcement.audience || announcement.targetRole)}
        </small>
        <em>
          {text(announcement.body || announcement.message, "No message").slice(
            0,
            115,
          )}
        </em>
      ...
      <span className="announcement-side">
        <span
          className={`status-dot-mini ${statusTone(announcement.status)}`}
          title={String(announcement.status || "sent")}
        />
        <button
          type="button"
      ...
      <div className="ba-table-scroll">
            ...
            {rows.map((announcement) => (
              <tr key={String(idOf(announcement))}>
                <td>
                  ...
                  <span>{announcement.createdBy || "Branch admin"}</span>
                </td>
                <td>
                  {dateLabel(
                    announcement.sentAt ||
                ...
                <td>
                  <Chip tone={priorityTone(announcement.priority)}>
                    {announcement.priority || "normal"}
                  </Chip>
                </td>
                <td>
                  <Chip tone={statusTone(announcement.status)}>
                ...
                <td>
                  {text(announcement.body || announcement.message).slice(
                    0,
                    120,
                  )}
                </td>
        ...
        {!rows.length && (
          <div className="ba-empty-table">
            No announcement matches your filters.
          </div>
        )}
      </div>
    </section>
    ...
    <div className="ba-sheet-backdrop" role="dialog" aria-modal="true">
      <section className="ba-sheet">
        <div className="ba-sheet-head">
          <div>
            <h2>Filters</h2>
            <p>Choose announcement status, audience, and priority.</p>
          </div>
    ...
    <div className="ba-sheet-backdrop" role="dialog" aria-modal="true">
      <section className="ba-sheet small">
        <div className="ba-sheet-head">
          <div>
            <h2>More</h2>
            <p>Views and refresh actions are here to keep the page compact.</p>
          </div>
        ...
        <div className="ba-menu-list">
          ...
          >
            <span>☰</span>
            <b>List view</b>
            <small>Compact announcement rows</small>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <b>Analytics</b>
            <small>
              {summary.shown}/{summary.total} shown · {summary.recipients}{" "}
              recipients
            </small>
          </button>
          <button type="button" onClick={onRefresh}>
            <span>↻</span>
            <b>Refresh</b>
            <small>Reload local branch announcements</small>
          </button>
        </div>
      </section>
    </div>
    ...
    <div className="ba-drawer-layer" role="dialog" aria-modal="true">
      ...
      <aside className="ba-drawer">
        <div className="ba-drawer-head">
          <div>
            <p>{mode === "edit" ? "Edit Announcement" : "New Announcement"}</p>
        ...
        <section className="ba-form-card">
          ...
          <p className="ba-hint">
            {candidatesCount} recipient(s) match this audience.
          </p>
        </section>
