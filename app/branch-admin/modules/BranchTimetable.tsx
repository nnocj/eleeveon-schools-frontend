...
const MODE_LABEL = "Branch Timetable";
const MODE_ICON = "🏫";
...
function idOf(value?: AnyRow | string | number | null): string {
  if (value && typeof value === "object") {
    return cleanId(value.id ?? value.payload?.id);
  }
...
function sessionTeacherId(session: AnyRow) {
  return cleanId(session.teacherId ?? session.staffId);
}
  ...
  async function save() {
    ...
    try {
      ...
      if (!timetableId) {
        ...
        timetableId = idOf(created as AnyRow);
      }
      ...
      const baseSessionPayload: Partial<ScheduleSession> = {
        ...
        title:
          form.title.trim() ||
          `${subjects.find((subject: AnyRow) => String(idOf(subject) ?? "") === String(form.subjectId ?? ""))?.name || "Session"}`,
        classId: cleanId(form.classId) || undefined,
      ...
      for (const day of selectedDays) {
        ...
        const matchingExistingSession = editingId
          ? sessions.find((session: AnyRow) => {
              ...
              if (cleanId(session.timetableId) !== timetableId)
                return false;
    ...
    return (
      ...
      >
        ...
        <section className="ba-state">
          <h2>Opening branch timetable...</h2>
          <p>
            Loading timetable sessions, teachers, classes, resources and
            conflicts.
          </p>
        </section>
      </main>
