...
export default function BranchWalletPage() {
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      const [txRows, paymentRows, settlementRows, withdrawalRows] =
        await Promise.all([
  ...
  const summary = useMemo(() => {
    ...
    const withdrawn = withdrawals
      .filter((row) =>
        ["paid", "approved", "settled"].includes(
          String(row.status || "").toLowerCase(),
        ),
      )
      .reduce((sum, row) => sum + n(row.amount), 0);
    const pendingWithdrawal = withdrawals
      .filter((row) =>
        ["pending", "requested", "processing", "review"].includes(
          String(row.status || "").toLowerCase(),
        ),
      )
      .reduce((sum, row) => sum + n(row.amount), 0);
  ...
  if (loading || accountLoading || settingsLoading)
    return (
      <State
        ...
        text="Loading branch inflows, settlements and withdrawal requests."
      />
    );
  if (!authenticated || !accountId)
    return (
      <State
        primary={primary}
        title="Redirecting..."
        text="Sign in before viewing branch wallet."
      />
    );
  if (!schoolId || !branchId)
    return (
      <State
        ...
        text="Wallet is branch-scoped. Select an active school and branch."
      />
    );
  ...
  return (
    ...
    >
      ...
      <section className="wallet-search-card">
        <span
          className={`status-dot-mini ${summary.available ? "green" : "gray"}`}
        />
        ...
        <button type="button" className="wallet-add-inline" onClick={load}>
          ↻
        </button>
        <button
          type="button"
      ...
      <section className="wallet-section-tabs">
        ...
        <button
          className={section === "settlements" ? "active" : ""}
      ...
      {view === "table" && (
        <Table rows={rows} currency={currency} section={section} />
      )}
      {view === "cards" && (
        <section className="wallet-list">
          ...
          {!rows.length && (
            <Empty
              title="No wallet records"
              text="Student fee transactions, settlements and withdrawal requests will appear here."
            />
          )}
        </section>
      )}
  ...
  return (
    <article className="wallet-card">
      ...
      <span className="wallet-main">
        ...
        <em>
          {row.purpose || row.direction || section} ·{" "}
          {dateLabel(
            row.paidAt || row.settledAt || row.requestedAt || row.createdAt,
          )}
        </em>
      </span>
      <span className="wallet-side">
        <Chip tone={toneForStatus(row.status)}>{row.status || "recorded"}</Chip>
      </span>
    </article>
  );
}
  ...
  return (
    <section className="wallet-table-card">
      <div className="wallet-table-scroll">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={String(idOf(row))}>
                <td>
                  <strong>{row.purpose || row.direction || section}</strong>
                  <span>
                    {row.payerName || row.recipientName || "Branch wallet"}
                  </span>
                </td>
                ...
                <td>{row.provider || row.channel || "manual"}</td>
                <td>
                  {dateLabel(
                    row.paidAt ||
                      row.settledAt ||
        ...
        {!rows.length && (
          <div className="wallet-empty-table">
  ...
  return (
    <section className="wallet-analysis-grid">
      <article className="wallet-analysis">
        ...
        <p>Estimated branch wallet balance.</p>
      </article>
      <article className="wallet-analysis">
        <span>Total Inflow</span>
        <strong>{money(summary.inflow, currency)}</strong>
        <p>Confirmed student-fee inflows.</p>
      </article>
      <article className="wallet-analysis">
        <span>Withdrawn</span>
        <strong>{money(summary.withdrawn, currency)}</strong>
        <p>Approved/paid withdrawals.</p>
      </article>
      <article className="wallet-analysis">
        <span>Pending</span>
        <strong>{money(summary.pendingWithdrawal, currency)}</strong>
        <p>Requested or processing withdrawals.</p>
      </article>
    </section>
  );
}
      ...
      <section className="wallet-sheet small">
        <div className="wallet-sheet-head">
          <div>
            <h2>Wallet</h2>
            <p>
              {money(summary.available, currency)} available ·{" "}
        ...
        <div className="wallet-menu-list">
          ...
          >
            <span>☰</span>
            <b>Cards view</b>
            <small>Compact wallet records</small>
          </button>
          <button
          ...
          >
            <span>☷</span>
            <b>Table view</b>
            <small>Dense wallet records</small>
          </button>
          <button
          ...
          >
            <span>◔</span>
            <b>Analytics</b>
            <small>Available, inflow and withdrawals</small>
          </button>
          <button
            onClick={() => {
              refresh();
          ...
          >
            <span>↻</span>
            <b>Refresh</b>
            <small>Reload wallet records</small>
          </button>
        </div>
      </section>
    </div>
