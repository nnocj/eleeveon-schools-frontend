...
export default function SettlementsPage() {
  ...
  useEffect(() => { if (accountLoading) return; if (!authenticated || !accountId) router.replace("/login"); }, [accountLoading, authenticated, accountId, router]);

  async function load() {
    if (!authenticated || !accountId || !schoolId || !branchId) { setLoading(false); return; }
    setLoading(true);
    try {
      const settlementRows = await listActiveLocal<AnyRow>("paymentSettlements" as any).catch(() => []);
      setRows(settlementRows.filter((row) => sameScope(row, accountId, schoolId, branchId)));
  ...
  const filtered = useMemo(() => {
    ...
    return rows
      ...
      .filter((row) => !q || [row.referenceNumber, row.providerReference, row.provider, row.status, row.note].join(" ").toLowerCase().includes(q))
      .sort((a,b) => n(b.settledAt || b.updatedAt || b.createdAt) - n(a.settledAt || a.updatedAt || a.createdAt));
  }, [query, rows, status]);

  const summary = useMemo(() => {
    const settled = filtered.filter((row) => ["settled", "paid", "success"].includes(String(row.status || "").toLowerCase())).reduce((sum, row) => sum + n(row.amount || row.netAmount), 0);
    const fees = filtered.reduce((sum, row) => sum + n(row.fee || row.providerFee || row.platformFee), 0);
    const pending = filtered.filter((row) => ["pending", "processing"].includes(String(row.status || "").toLowerCase())).reduce((sum, row) => sum + n(row.amount || row.netAmount), 0);
    return { count: filtered.length, settled, fees, pending, net: Math.max(0, settled - fees) };
  }, [filtered]);

  if (loading || accountLoading || settingsLoading) return <State primary={primary} title="Opening settlements..." text="Loading branch settlement records." />;
  if (!authenticated || !accountId) return <State primary={primary} title="Redirecting..." text="Sign in before viewing settlements." />;
  if (!schoolId || !branchId) return <State primary={primary} title="Select branch context" text="Settlements are branch-scoped." />;

  return (
    <main className="wallet-page" style={{ "--wallet-primary": primary } as React.CSSProperties}>
      <style>{css}</style>
      <section className="wallet-search-card"><span className={`status-dot-mini ${summary.settled ? "green" : "gray"}`} /><label className="wallet-search"><span>⌕</span><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search settlements..." /></label><button className="wallet-add-inline" onClick={load}>↻</button><button className={`wallet-filter-button ${status !== "all" ? "active" : ""}`} onClick={()=>setFilterOpen(true)}><SliderIcon />{status !== "all" && <b>1</b>}</button><button className="wallet-icon-button" onClick={()=>setMoreOpen(true)}>⋯</button></section>
      {status !== "all" && <section className="wallet-filter-chips"><button onClick={()=>setStatus("all")}>Status: {status} ×</button></section>}
      {view === "analytics" && <Analytics summary={summary} currency={currency} />}
      {view === "table" && <Table rows={filtered} currency={currency} />}
      {view === "cards" && <section className="wallet-list">{filtered.map((row)=><SettlementCard key={String(idOf(row))} row={row} currency={row.currencyCode || currency} />)}{!filtered.length && <Empty title="No settlements found" text="Settlements from provider/platform wallet will appear here." />}</section>}
      {filterOpen && <Filter status={status} setStatus={setStatus} close={()=>setFilterOpen(false)} />}
      {moreOpen && <More view={view} setView={setView} summary={summary} currency={currency} close={()=>setMoreOpen(false)} refresh={load} />}
    </main>
  );
}

function SettlementCard({row,currency}:{row:AnyRow;currency:string}){return <article className="wallet-card"><span className="wallet-avatar">🏦</span><span className="wallet-main"><strong>{money(row.amount || row.netAmount,currency)}</strong><small>{row.referenceNumber || row.providerReference || "Settlement"}</small><em>{row.provider || "provider"} · {dateLabel(row.settledAt || row.createdAt)}</em></span><span className="wallet-side"><Chip tone={toneForStatus(row.status)}>{row.status || "pending"}</Chip></span></article>}
function Table({rows,currency}:{rows:AnyRow[];currency:string}){return <section className="wallet-table-card"><div className="wallet-table-scroll"><table><thead><tr><th>Settlements ({rows.length})</th><th>Gross</th><th>Fees</th><th>Net</th><th>Status</th><th>Reference</th><th>Date</th></tr></thead><tbody>{rows.map((row)=><tr key={String(idOf(row))}><td><strong>{row.provider || "Provider"}</strong><span>{row.note || "Settlement"}</span></td><td>{money(row.grossAmount || row.amount,currency)}</td><td>{money(row.fee || row.providerFee || row.platformFee,currency)}</td><td>{money(row.netAmount || row.amount,currency)}</td><td><Chip tone={toneForStatus(row.status)}>{row.status || "pending"}</Chip></td><td>{row.referenceNumber || row.providerReference || "—"}</td><td>{dateLabel(row.settledAt || row.createdAt)}</td></tr>)}</tbody></table>{!rows.length && <div className="wallet-empty-table">No settlement matches.</div>}</div></section>}
function Analytics({summary,currency}:{summary:AnyRow;currency:string}){return <section className="wallet-analysis-grid"><article className="wallet-analysis"><span>Settled</span><strong>{money(summary.settled,currency)}</strong><p>Confirmed settlement amount.</p></article><article className="wallet-analysis"><span>Pending</span><strong>{money(summary.pending,currency)}</strong><p>Waiting settlement.</p></article><article className="wallet-analysis"><span>Fees</span><strong>{money(summary.fees,currency)}</strong><p>Provider/platform fees.</p></article><article className="wallet-analysis"><span>Net</span><strong>{money(summary.net,currency)}</strong><p>Net settled amount.</p></article></section>}
function Filter({status,setStatus,close}:{status:string;setStatus:(v:string)=>void;close:()=>void}){return <div className="wallet-sheet-backdrop"><section className="wallet-sheet small"><div className="wallet-sheet-head"><div><h2>Filters</h2><p>Filter settlement status.</p></div><button onClick={close}>✕</button></div><div className="wallet-form"><label><span>Status</span><select value={status} onChange={(e)=>setStatus(e.target.value)}><option value="all">All</option><option value="pending">Pending</option><option value="processing">Processing</option><option value="settled">Settled</option><option value="failed">Failed</option></select></label></div><div className="wallet-sheet-actions"><button onClick={()=>setStatus("all")}>Reset</button><button className="primary" onClick={close}>Apply</button></div></section></div>}
function More({view,setView,summary,currency,close,refresh}:{view:ViewMode;setView:(v:ViewMode)=>void;summary:AnyRow;currency:string;close:()=>void;refresh:()=>void}){return <div className="wallet-sheet-backdrop"><section className="wallet-sheet small"><div className="wallet-sheet-head"><div><h2>Settlements</h2><p>{money(summary.settled,currency)} settled.</p></div><button onClick={close}>✕</button></div><div className="wallet-menu-list"><button className={view==="cards"?"active":""} onClick={()=>{setView("cards");close();}}><span>☰</span><b>Cards</b><small>Compact records</small></button><button className={view==="table"?"active":""} onClick={()=>{setView("table");close();}}><span>☷</span><b>Table</b><small>Dense records</small></button><button className={view==="analytics"?"active":""} onClick={()=>{setView("analytics");close();}}><span>◔</span><b>Analytics</b><small>Settlement totals</small></button><button onClick={()=>{refresh();close();}}><span>↻</span><b>Refresh</b><small>Reload settlements</small></button></div></section></div>}
