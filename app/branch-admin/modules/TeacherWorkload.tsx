...
export function TeacherWorkload({ teachers, sessions, conflicts }: { teachers: AnyRow[]; sessions: AnyRow[]; conflicts: AnyRow[] }) {
  const rows = useMemo(() => {
    return teachers
      .map((teacher) => {
        const teacherId = cleanId(idOf(teacher));
        const teacherSessions = sessions.filter((session) => sessionTeacherId(session) === teacherId);
        const minutes = teacherSessions.reduce((sum, session) => sum + Math.max(0, endMinute(session) - startMinute(session)), 0);
        const hours = Math.round((minutes / 60) * 10) / 10;
        const subjectCount = new Set(teacherSessions.map(sessionSubjectId).filter(Boolean)).size;
        const classCount = new Set(teacherSessions.map(sessionClassId).filter(Boolean)).size;
        const conflictCount = conflicts.filter(
          (conflict) => cleanId(conflict.teacherLocalId || conflict.teacherId) === teacherId
        ).length;
  ...
  const averageHours = activeTeachers ? Math.round((totalHours / activeTeachers) * 10) / 10 : 0;
  ...
  return (
    <section className="ba-section">
      ...
      <div className="ba-mini-grid ba-workload-summary">
        <MiniStat label="Total Teaching Hours" value={`${totalHours.toFixed(1)}h`} />
        <MiniStat label="Average Active Load" value={`${averageHours.toFixed(1)}h`} />
          ...
          <article key={String(row.id || row.name)} className="ba-card">
            <div className="ba-card-top">
              ...
              <div className="ba-card-main">
                <h3>{row.name}</h3>
                <p>{row.sessions} session(s) · {row.hours.toFixed(1)} teaching hour(s)</p>
                <div className="ba-chip-row">
                  <Chip tone={workloadTone(row.hours)}>{row.hours.toFixed(1)}h load</Chip>
                  <Chip>{row.subjectCount} subject(s)</Chip>
                  <Chip>{row.classCount} class(es)</Chip>
                  {row.conflictCount ? <Chip tone="orange">{row.conflictCount} conflict(s)</Chip> : <Chip tone="green">No conflicts</Chip>}
                </div>
              </div>
            </div>
          </article>
...
export function TeacherLoadAnalytics({ teachers, sessions }: { teachers: AnyRow[]; sessions: AnyRow[] }) {
  const rows = useMemo(() => {
    return teachers
      .map((teacher) => {
        const teacherId = cleanId(idOf(teacher));
        const teacherSessions = sessions.filter((session) => sessionTeacherId(session) === teacherId);
        const minutes = teacherSessions.reduce((sum, session) => sum + Math.max(0, endMinute(session) - startMinute(session)), 0);
        return { label: teacherName(teachers, teacherId), value: Math.round((minutes / 60) * 10) / 10 };
      })
