...
export default function CumulativeBookSummaryPage({
  ...
  const cards = [
    ...
    [
      "First Average",
      summary.firstAverage == null
        ? "-"
        : `${formatNumber(summary.firstAverage, 1)}%`,
    ],
    [
      "Latest Average",
      summary.latestAverage == null
        ? "-"
        : `${formatNumber(summary.latestAverage, 1)}%`,
    ],
    [
      "Best Average",
      summary.bestAverage == null
        ? "-"
        : `${formatNumber(summary.bestAverage, 1)}%`,
    ],
    [
      "Cumulative Average",
      summary.cumulativeAverage == null
        ? "-"
        : `${formatNumber(summary.cumulativeAverage, 1)}%`,
    ],
    [
      "Cumulative GPA",
      summary.cumulativeGPA == null
        ? "-"
        : formatNumber(summary.cumulativeGPA, 2),
    ],
    [
      "Best Position",
      summary.bestPosition == null ? "-" : ordinal(summary.bestPosition),
    ],
    [
      "Latest Position",
      summary.latestPosition == null ? "-" : ordinal(summary.latestPosition),
    ],
  ...
  return (
    <section
      className="print-page cumulative-book-page cumulative-book-summary"
      style={page}
    >
      ...
      <div style={{ marginTop: 18 }}>
        ...
        >
          <Info
            ...
            text="Each report-card page after this section represents one published academic period. The page uses the same selected student report template, so the book remains visually consistent from period to period."
          />
          <Info
            title="How cumulative values are estimated"
            text="The summary values are derived from the available published report snapshots. If a period has no published snapshot, it is not included in this booklet."
          />
        </div>
      </div>
...
function Info({ title, text }: { title: string; text: string }) {
  return (
    ...
    >
      <div style={{ fontSize: 10.5, fontWeight: 950, color: "#111827" }}>
        {title}
      </div>
      <p
        style={{
