...
export default function AssessmentEntriesPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setComponents(comp.filter(x => !x.isDeleted));
      setEnrollments(enr.filter(x => !x.isDeleted));
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(
      i => Number(i.assessmentStructureId) === Number(structureId)
    );
  }, [items, structureId]);
