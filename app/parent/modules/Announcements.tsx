...
/**
 * app/parent/modules/Announcements.tsx
 * ---------------------------------------------------------
...
export default function Announcements() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [ann, rec] = await Promise.all([
      ...
      const parentAnnouncements = (ann as AnyRow[]).filter((item) => {
        ...
        return (
          ...
          audience.includes("guardian") ||
          targetRoles.includes("parent") ||
          audience === "whole_branch" ||
          childClassIds.has(Number(item.classId || item.scopeId))
        );
      });
  ...
  }), [announcements, recipients, children]);

  const acknowledge = async (item: AnyRow) => {
    const table = (db as any).announcementRecipients;
    if (!table) return;

    const existing = recipients.find((r) => r.announcementId === item.id && (r.role === "parent" || r.email === parent?.email));
  ...
  if (loading || accountLoading || settingsLoading || contextLoading) {
    return <main className="pp-page" style={{"--pp-primary": primary} as React.CSSProperties}><style>{css}</style><section className="pp-state"><h2>Opening announcements...</h2><p>Loading parent notices.</p></section></main>;
  }
  ...
  return (
    <main className="pp-page" style={{"--pp-primary": primary} as React.CSSProperties}>
      ...
      <section className="pp-hero">
        <div className="pp-hero-left"><div className="pp-icon">📣</div><div className="pp-title"><p>Parent Communication</p><h2>Announcements</h2><span>{rowName(parent || {})} · Receive only</span></div></div>
      ...
      {view === "analytics" && (
        <section className="pp-section pp-breakdown-grid">
          {["parents","whole_branch","all_guardians","students","teachers"].map((audience) => {
            ...
            return <article key={audience} className="pp-breakdown"><strong>{audience.replaceAll("_"," ")}</strong><div className="pp-bar"><div style={{width:`${pct}%`}} /></div><div className="pp-chip-row"><Chip tone="blue">{count}</Chip><Chip>{pct}%</Chip></div></article>;
          })}
        </section>
      )}

      {view === "table" && (
        <section className="pp-panel"><div className="pp-table-wrap"><table className="pp-table"><thead><tr><th>Date</th><th>Title</th><th>Audience</th><th>Priority</th><th>Status</th><th>Action</th></tr></thead><tbody>
          {filtered.map((a) => <tr key={a.id}><td>{dateLabel(a.sentAt || a.publishAt || a.createdAt)}</td><td><strong>{a.title}</strong><br/><span>{text(a.body || a.message).slice(0, 80)}</span></td><td>{a.audience || a.targetRole || "branch"}</td><td><Chip tone={a.priority === "urgent" ? "red" : a.priority === "high" ? "orange" : "blue"}>{a.priority || "normal"}</Chip></td><td><Chip tone={a.status === "draft" ? "gray" : "green"}>{a.status || "sent"}</Chip></td><td><button className="pp-btn" onClick={() => acknowledge(a)}>Acknowledge</button></td></tr>)}
      ...
      {view === "cards" && (
        <section className="pp-section"><div className="pp-list">
          {filtered.map((a) => <article key={a.id} className="pp-card"><div className="pp-card-top"><div className="pp-avatar">📣</div><div className="pp-card-main"><h3>{a.title}</h3><p>{dateLabel(a.sentAt || a.publishAt || a.createdAt)} · {a.audience || a.targetRole || "branch"}</p><div className="pp-chip-row"><Chip tone={a.priority === "urgent" ? "red" : a.priority === "high" ? "orange" : "blue"}>{a.priority || "normal"}</Chip><Chip tone={a.status === "draft" ? "gray" : "green"}>{a.status || "sent"}</Chip><button className="pp-btn" onClick={() => acknowledge(a)}>Acknowledge</button></div></div></div><p className="pp-message">{a.body || a.message}</p></article>)}
