...
export default function SchoolBranchFinanceMonitor() {
  const data = useSchoolFinanceData();
  const [filter, setFilter] = useState("all");

  const rows = useMemo(() => {
    if (filter === "risk") return data.branchSummaries.filter((row) => row.balance < 0 || row.payrollDue > 0 || row.approvalsNeeded > 0);
    if (filter === "healthy") return data.branchSummaries.filter((row) => row.balance >= 0 && row.payrollDue <= 0 && row.approvalsNeeded <= 0);
    return data.branchSummaries;
  }, [data.branchSummaries, filter]);

  const maxBalance = Math.max(1, ...data.branchSummaries.map((row) => Math.abs(row.balance)));

  if (data.loading) {
    return <main className="saf-page" style={{ "--saf-primary": data.primary } as React.CSSProperties}><style>{css}</style><section className="saf-loading">Loading branch finance monitor...</section></main>;
  }

  return (
    <main className="saf-page" style={{ "--saf-primary": data.primary } as React.CSSProperties}>
      <style>{css}</style>
      <section className="saf-hero"><div><p className="saf-eyebrow">Branch Finance Monitor</p><h1>Monitor branch finance without taking over branch work</h1><p>See which branches are healthy, which branches need funds, and which records need school-level attention.</p></div><div className="saf-actions"><button className="saf-btn-light" onClick={() => data.reload()}>Refresh</button></div></section>
      {data.notice ? <div className="saf-notice">{data.notice}</div> : null}

      <section className="saf-stat-grid">
        <StatCard label="Branches At Risk" value={data.branchSummaries.filter((row)=> row.balance < 0 || row.payrollDue > 0).length} detail="negative balance or payroll due" icon="⚠️" tone="orange" />
        <StatCard label="Healthy Branches" value={data.branchSummaries.filter((row)=> row.balance >= 0 && row.payrollDue <= 0).length} detail="no immediate pressure" icon="✅" tone="green" />
        <StatCard label="Pending Payments" value={data.totals.pendingPayments} detail="not yet confirmed paid" icon="🏦" tone={data.totals.pendingPayments ? "orange" : "green"} />
        <StatCard label="Approvals Needed" value={data.totals.approvals} detail="records to review" icon="📝" tone={data.totals.approvals ? "orange" : "green"} />
      </section>

      <section className="saf-card">
        <div className="saf-head"><div><h2>Branch risk table</h2><p>Use this to supervise branch finance and identify where to intervene.</p></div><div className="saf-tabs"><button className={filter === "all" ? "active" : ""} onClick={()=>setFilter("all")}>All</button><button className={filter === "risk" ? "active" : ""} onClick={()=>setFilter("risk")}>Risk</button><button className={filter === "healthy" ? "active" : ""} onClick={()=>setFilter("healthy")}>Healthy</button></div></div>
        <div className="saf-table-wrap">
          <table className="saf-table"><thead><tr><th>Branch</th><th>Income</th><th>Expense</th><th>Payroll Due</th><th>Payment Activity</th><th>Estimated Balance</th><th>Risk</th></tr></thead><tbody>{rows.map((row)=>{const risk = row.balance < 0 ? "Deficit" : row.payrollDue > 0 ? "Payroll due" : row.approvalsNeeded > 0 ? "Review" : "Healthy"; return <tr key={row.id}><td><strong>{row.name}</strong><br/><small>{row.code || "No code"}</small></td><td>{money(row.income + row.studentFees)}</td><td>{money(row.expenses)}</td><td>{money(row.payrollDue)}</td><td>{row.paymentCount} records · {row.pendingPayments} pending</td><td className={row.balance >= 0 ? "saf-money-pos" : "saf-money-neg"}>{money(row.balance)}</td><td><Chip tone={risk === "Healthy" ? "green" : risk === "Deficit" ? "red" : "orange"}>{risk}</Chip></td></tr>})}</tbody></table>
        </div>
        <div className="saf-mobile-cards">{rows.map((row)=><article className="saf-branch-card" key={row.id}><div className="saf-head"><div><h3>{row.name}</h3><p>{row.paymentCount} payment records · {row.pendingPayments} pending</p></div><Chip tone={row.balance >= 0 ? "green" : "red"}>{money(row.balance)}</Chip></div><div className="saf-kpi-grid"><div className="saf-kpi"><span>Income</span><strong>{money(row.income + row.studentFees)}</strong></div><div className="saf-kpi"><span>Payroll Due</span><strong>{money(row.payrollDue)}</strong></div></div></article>)}</div>
        {!rows.length ? <EmptyState title="No matching branches" detail="Try another filter or refresh after sync." /> : null}
      </section>

      <section className="saf-card"><div className="saf-head"><div><h2>Balance pressure chart</h2><p>Large negative bars need funding or branch-admin follow-up.</p></div></div><div className="saf-bars">{data.branchSummaries.map((row)=><Bar key={row.id} label={row.name} value={Math.abs(row.balance)} max={maxBalance} tone={row.balance >= 0 ? "green" : "red"} />)}</div></section>
    </main>
  );
}
