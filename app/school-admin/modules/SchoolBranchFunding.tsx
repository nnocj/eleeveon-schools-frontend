...
export default function SchoolBranchFunding() {
  ...
  const selectedBranch = data.branchSummaries.find((row) => String(row.id) === branchId);
  const priorityBranches = useMemo(() => [...data.branchSummaries].sort((a,b)=> b.payrollDue - a.payrollDue).slice(0,5), [data.branchSummaries]);

  async function createFundingRecord() {
    setMessage("");
    const numericAmount = Number(amount);
    if (!data.accountId || !data.schoolId || !selectedBranch) return setMessage("Select a valid branch first.");
    if (!Number.isFinite(numericAmount) || numericAmount <= 0) return setMessage("Enter a valid amount to allocate.");
    ...
    const payload = {
      ...
      title: `School allocation to ${selectedBranch.name}`,
      description: note || "School admin branch funding allocation.",
      amount: numericAmount,
      paymentMethod: method,
      date: new Date().toISOString().slice(0, 10),
      source: "school_admin_allocation",
      receivedBy: "Branch Admin",
      referenceNumber: `SAF-${now}`,
      createdAt: now,
      updatedAt: now,
      version: 1,
      deviceId: typeof localStorage !== "undefined" ? localStorage.getItem("deviceId") || "local-device" : "local-device",
      synced: "pending",
      isDeleted: false,
    };
    ...
    try {
      setSaving(true);
      const incomeTable = getTable("income", "incomes");
      const txTable = getTable("paymentTransactions");

      if (incomeTable?.add) await incomeTable.add(payload);
      if (txTable?.add) {
        await txTable.add({
          ...payload,
          purpose: "other",
          ...
          status: "pending",
          direction: "inflow",
          recipientName: selectedBranch.name,
          note: payload.description,
          paidAt: undefined,
        });
      }

      setMessage(`Funding record created for ${selectedBranch.name}. It will sync when your local sync runs.`);
      setAmount("");
      setNote("");
      await data.reload();
    } catch (err: any) {
      setMessage(err?.message || "Unable to create branch funding record.");
    } finally {
      setSaving(false);
    }
  }
  ...
  if (data.loading) {
    return <main className="saf-page" style={{ "--saf-primary": data.primary } as React.CSSProperties}><style>{css}</style><section className="saf-loading">Loading branch funding...</section></main>;
  }
  ...
  return (
    <main className="saf-page" style={{ "--saf-primary": data.primary } as React.CSSProperties}>
      ...
      <section className="saf-hero">
        <div><p className="saf-eyebrow">Branch Funding</p><h1>Send money to branch admins</h1><p>Create school-level funding allocations for branches. Branch admins can then use the funds for salaries, expenses and local operations.</p></div>
        <div className="saf-actions"><button className="saf-btn-light" onClick={() => data.reload()}>Refresh</button></div>
      </section>

      {data.notice || message ? <div className="saf-notice">{message || data.notice}</div> : null}

      <section className="saf-stat-grid">
        <StatCard label="Active Branches" value={`${data.totals.activeBranches}/${data.totals.branches}`} detail="available for allocation" icon="🏫" tone="blue" />
        <StatCard label="Payroll Due" value={money(data.totals.payrollDue)} detail="highest funding pressure" icon="🧾" tone={data.totals.payrollDue ? "orange" : "green"} />
        <StatCard label="Pending Reviews" value={data.totals.approvals} detail="funding/payroll/payment records" icon="✅" tone={data.totals.approvals ? "orange" : "green"} />
        <StatCard label="Branch Balance" value={money(data.totals.branchBalance)} detail="estimated across branches" icon="🏦" tone={toneFor(data.totals.branchBalance)} />
      </section>

      <section className="saf-grid">
        <section className="saf-card">
          <div className="saf-head"><div><h2>New branch funding</h2><p>This records money allocated from school admin to a selected branch.</p></div><Chip tone="purple">school allocation</Chip></div>

          <div className="saf-form">
            <div className="saf-form-row">
              <div className="saf-field"><label>Branch</label><select className="saf-select" value={branchId} onChange={(e)=>setBranchId(e.target.value)}><option value="">Select branch</option>{data.branchSummaries.map((branch)=><option key={branch.id} value={branch.id}>{branch.name} · due {money(branch.payrollDue)}</option>)}</select></div>
              <div className="saf-field"><label>Amount</label><input className="saf-input" inputMode="decimal" value={amount} onChange={(e)=>setAmount(e.target.value)} placeholder="0.00" /></div>
            </div>
            <div className="saf-form-row">
              <div className="saf-field"><label>Method</label><select className="saf-select" value={method} onChange={(e)=>setMethod(e.target.value)}><option value="momo">Mobile Money</option><option value="bank">Bank</option><option value="cash">Cash</option><option value="manual">Manual</option></select></div>
              <div className="saf-field"><label>Suggested amount</label><input className="saf-input" readOnly value={selectedBranch ? money(Math.max(selectedBranch.payrollDue, 0)) : "Select branch"} /></div>
            </div>
            <div className="saf-field"><label>Note</label><textarea className="saf-textarea" value={note} onChange={(e)=>setNote(e.target.value)} placeholder="Purpose, approval note, salary support, operational support..." /></div>
            <button className="saf-btn" onClick={createFundingRecord} disabled={saving}>{saving ? "Saving..." : "Create funding record"}</button>
          </div>
        </section>

        <aside className="saf-card">
          <div className="saf-head"><div><h2>Funding priority</h2><p>Branches with the largest unpaid payroll should be funded first.</p></div></div>
          <div className="saf-mini-list">
            {priorityBranches.map((branch)=>(
              <article className="saf-mini-item" key={branch.id}><div><strong>{branch.name}</strong><span>Payroll due {money(branch.payrollDue)} · balance {money(branch.balance)}</span></div><Chip tone={branch.payrollDue ? "orange" : "green"}>{branch.payrollDue ? "fund" : "ok"}</Chip></article>
            ))}
            {!priorityBranches.length ? <EmptyState title="No branches yet" detail="Sync or create branches before allocating money." /> : null}
          </div>
        </aside>
      </section>
    </main>
