...
export default function SchoolFinanceOverview() {
  ...
  const [view, setView] = useState<"branches" | "charts">("branches");

  const maxBranchIncome = Math.max(1, ...data.branchSummaries.map((row) => row.income + row.studentFees));
  const maxPayroll = Math.max(1, ...data.branchSummaries.map((row) => row.payrollDue + row.payrollPaid));

  if (data.loading) {
    return <main className="saf-page" style={{ "--saf-primary": data.primary } as React.CSSProperties}><style>{css}</style><section className="saf-loading">Loading school-wide finance overview...</section></main>;
  }
  ...
  return (
    <main className="saf-page" style={{ "--saf-primary": data.primary } as React.CSSProperties}>
      ...
      <section className="saf-hero">
        <div>
          <p className="saf-eyebrow">School Finance Control</p>
          <h1>School-wide finance overview</h1>
          <p>Monitor all branches from one place. The school admin sees totals, risks and branch performance, while branch admins handle daily payroll and payment execution.</p>
        </div>
        <div className="saf-actions">
          <button className="saf-btn-light" onClick={() => data.reload()}>Refresh</button>
        </div>
      </section>
      ...
      <section className="saf-stat-grid">
        <StatCard label="Branch Income" value={money(data.totals.income + data.totals.fees)} detail="income + student fee records" icon="💰" tone="green" />
        <StatCard label="Branch Expenses" value={money(data.totals.expenses)} detail="recorded branch expenses" icon="📉" tone={data.totals.expenses ? "orange" : "gray"} />
        <StatCard label="Payroll Due" value={money(data.totals.payrollDue)} detail={`${data.payrollRows.length} staff payroll records`} icon="🧾" tone={data.totals.payrollDue ? "orange" : "green"} />
        <StatCard label="Net Position" value={money(data.totals.branchBalance)} detail={`${data.totals.activeBranches}/${data.totals.branches} active branches`} icon="🏦" tone={toneFor(data.totals.branchBalance)} />
      </section>
      ...
      <section className="saf-grid">
        <section className="saf-card">
          <div className="saf-head">
            <div>
              <h2>Branch finance summary</h2>
              <p>Every branch under the assigned school is counted here.</p>
            </div>
            ...
            <>
              <div className="saf-table-wrap">
                <table className="saf-table">
                  <thead><tr><th>Branch</th><th>Income</th><th>Expenses</th><th>Payroll Due</th><th>Payments</th><th>Net</th><th>Status</th></tr></thead>
                  <tbody>
                    {data.branchSummaries.map((row) => (
                      <tr key={row.id}>
                        <td><strong>{row.name}</strong><br/><small>{row.code || "No code"}</small></td>
                        <td className="saf-money-pos">{money(row.income + row.studentFees)}</td>
                        <td>{money(row.expenses)}</td>
                        <td>{money(row.payrollDue)}</td>
                        <td>{row.paidPayments} paid · {row.pendingPayments} pending</td>
                        <td className={row.balance >= 0 ? "saf-money-pos" : "saf-money-neg"}>{money(row.balance)}</td>
                        <td><Chip tone={row.approvalsNeeded ? "orange" : "green"}>{row.approvalsNeeded ? `${row.approvalsNeeded} review` : "clean"}</Chip></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="saf-mobile-cards">
                {data.branchSummaries.map((row) => (
                  <article className="saf-branch-card" key={row.id}>
                    <div className="saf-head"><div><h3>{row.name}</h3><p>{row.code || "No branch code"}</p></div><Chip tone={row.balance >= 0 ? "green" : "red"}>{money(row.balance)}</Chip></div>
                    <div className="saf-kpi-grid"><div className="saf-kpi"><span>Income</span><strong>{money(row.income + row.studentFees)}</strong></div><div className="saf-kpi"><span>Payroll Due</span><strong>{money(row.payrollDue)}</strong></div></div>
                    <p>{row.paidPayments} paid payments · {row.pendingPayments} pending · {row.approvalsNeeded} needs review</p>
                  </article>
                ))}
              </div>

              {!data.branchSummaries.length ? <EmptyState title="No branch finance found" detail="Create branches or sync branch records to see school-wide finance summaries." /> : null}
            </>
          ) : (
            <div className="saf-bars">
              {data.branchSummaries.map((row) => <Bar key={`inc-${row.id}`} label={`${row.name} income`} value={row.income + row.studentFees} max={maxBranchIncome} tone="green" />)}
              {data.branchSummaries.map((row) => <Bar key={`pay-${row.id}`} label={`${row.name} payroll`} value={row.payrollDue + row.payrollPaid} max={maxPayroll} tone="orange" />)}
            </div>
          )}
        </section>

        <aside className="saf-card">
          <div className="saf-head"><div><h2>School admin position</h2><p>Use this page for control and decisions, not daily branch cashier work.</p></div></div>
          <div className="saf-mini-list">
            <article className="saf-mini-item"><div><strong>Branch admins execute</strong><span>Staff salaries, fee collection, expenses and local payments.</span></div><Chip tone="blue">branch</Chip></article>
            <article className="saf-mini-item"><div><strong>School admin controls</strong><span>Funding, approvals, monitoring and branch accountability.</span></div><Chip tone="purple">school</Chip></article>
            <article className="saf-mini-item"><div><strong>Pending review</strong><span>{data.totals.approvals} payment/payroll records need attention.</span></div><Chip tone={data.totals.approvals ? "orange" : "green"}>{data.totals.approvals}</Chip></article>
          </div>
        </aside>
      </section>
    </main>
